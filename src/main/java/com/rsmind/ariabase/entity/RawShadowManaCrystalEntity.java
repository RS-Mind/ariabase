...
public class RawShadowManaCrystalEntity extends ProjectileItemEntity {
	public int explosionPower = 1;
	   
	public RawShadowManaCrystalEntity(EntityType<? extends RawShadowManaCrystalEntity> p_i50159_1_, World p_i50159_2_) {
		super(p_i50159_1_, p_i50159_2_);
		}

	public RawShadowManaCrystalEntity(World worldIn, LivingEntity throwerIn) {
		super(EntityType.SNOWBALL, throwerIn, worldIn);
		}

   public RawShadowManaCrystalEntity(World worldIn, double x, double y, double z) {
   ...
   private IParticleData makeParticle() {
      ...
      return (IParticleData)(itemstack.isEmpty() ? new ItemParticleData(ParticleTypes.ITEM, new ItemStack(ModItems.RAW_MANA_CRYSTAL_SHADOW.get())) : new ItemParticleData(ParticleTypes.ITEM, itemstack));
   }
   ...
   protected void onImpact(RayTraceResult result) {
      ...
      if (!this.world.isRemote) {
    	  ...
    	  if(this.world.getBlockState(pos1).getBlock() == Blocks.IRON_BLOCK) {
    		  Block block = ModBlocks.MANASTEEL_ORE_SHADOW.get();
    		  BlockState state = block.getDefaultState();
    		  this.world.setBlockState(pos1, state);
    	  }
    	  else if(this.world.getBlockState(pos1).getBlock() == Blocks.GLASS) {
    		  Block block = ModBlocks.INFUSED_GLASS_SHADOW.get();
    		  BlockState state = block.getDefaultState();
    		  this.world.setBlockState(pos1, state);
    	  }
